...
for x in range(len(list)):
    for y in range(len(list)):
        data_without_noise.append([x,y,list[x]*list[y]])
print data_without_noise

data_with_noise=[]
for data in data_without_noise:
    for i in range(2):
        data_with_noise.append([data[0]+random.random()/4,data[1]+random.random()/4,data[2]+random.random()])
for data in data_without_noise:
    for i in range(2):
        data_with_noise.append([data[0]+random.random()/4,data[1]+random.random()/4,data[2]+random.random()])
...
colors =(np.array(data_with_noise)[:,2]/100)
